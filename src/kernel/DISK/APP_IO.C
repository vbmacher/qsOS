/*
   ...
   Modul bol modifikovany z emulatora QFS
...
static char a_fname[8];
static char a_fext[3];
static char a_filename[12];
...
int fread_app(FileHandle *handle, int nsize, char *buffer)
{
  int fbytes;
  if (!handle->ilink) return 0;
  if (!handle->fsize) return 0;

  fbytes = read_log(handle->fname,handle->fext,handle->ilink_d,nsize,handle->fpos, buffer);
  handle->fpos += fbytes;
  return fbytes;
}

// Funkcia zapise otvoreny subor z handle
int fwrite_app(FileHandle *handle, int nsize, char *buffer)
{
  unsigned int fbytes;
  if (!handle->ilink) return 0;

  fbytes = write_log(handle->fname,handle->fext,handle->ilink_d,nsize,handle->fpos, buffer);
  if (fbytes) {
...
{
  textattr(atrINIT);
  cprintf("\nInitializing disk control (level 2)...");
  textattr(atrBORDER);
  PATH = 0;
  return 0;
}
...
{
  textattr(atrINIT);
  cprintf("\nDestroying disk control (level 2)...");
  textattr(atrBORDER);
  PATH = 0;
}