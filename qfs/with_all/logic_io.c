...
int format_log()
{
  ...
  decode_lp(0);
  write_phy(PP.HEAD, PP.TRACK, PP.SECTOR, 1, IN);
  IN->inodes[0].ftype = 2;
...
unsigned int read_log(char fname[8], char fext[3], unsigned char idir, unsigned int dcount, unsigned long dpos, char *buf)
{
  int LS;                  // pomocne logicke sektory
...
read_more:
  ...
  if (pos_db >= file->info.dcount)
    ...
    if (file->info.dcount < DATA_COUNT) {
      ...
      buffer += ((unsigned int)(file->info.dcount-pos_db));
      dcount -= ((unsigned int)(file->info.dcount-pos_db));
      db++;
      pos_db = 0;
      goto ex;
    }
...
int Init_LOG()
{
  if ((dir = (dbdir *)malloc(sizeof(dbdir))) == NULL) return FAIL;
  if ((file = (dbfile *)malloc(sizeof(dbfile))) == NULL) {
    free(dir);
    return FAIL;
  }
  if ((IN = (inodes_sec *)malloc(sizeof(inodes_sec))) == NULL) {
    free(file);
    free(dir);
    return FAIL;
  }
